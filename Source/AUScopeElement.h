...
public:
	...
	[[nodiscard]] AUElement* SafeGetElement(UInt32 elementIndex) const
	{
		...
		AUSDK_ThrowIf(element == nullptr, kAudioUnitErr_InvalidElement);
		return element;
	}
	[[nodiscard]] AUIOElement* GetIOElement(UInt32 elementIndex) const
	{
		...
		AUSDK_ThrowIf(ioel == nullptr, kAudioUnitErr_InvalidElement);
		return ioel;
	}
	...
	[[nodiscard]] bool RestoreElementNames(CFDictionaryRef inNameDict) const;
