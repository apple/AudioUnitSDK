...
static CFStringRef kCPULoadString = nullptr;       // NOLINT non-const global
	  ...
	  mInitNumOutputEls(numOutputElements), mInitNumGroupEls(numGroupElements)
{
	...
	std::call_once(sAUBaseCFStringsInitialized, []() {
		...
		kCPULoadString = CFSTR(kAUPresetCPULoadKey);             // NOLINT
	...
	AudioUnitScope inScope, AudioUnitElement inElement)
{
	...
	case kAudioUnitScope_Global: // global stream description is an alias for that of output 0
		...
		break;
	default:
		AUSDK_Throw(kAudioUnitErr_InvalidScope);
	}
...
const char* AUBase::GetLoggingString()
{
	if (!mLogString.empty()) {
