...
#define AUSDK_LogError(...) os_log_error(AUSDK_LOG_OBJECT, __VA_ARGS__) /* NOLINT macro */

#define AUSDK_ThrowIf(cond, err) /* NOLINT(cppcoreguidelines-macro-usage) */                       \
	do {                                                                                           \
		if (cond) {                                                                                \
			AUSDK_LogError("throwing %d", static_cast<int>(err));                                  \
			throw ausdk::AUException{ err };                                                       \
		}                                                                                          \
	} while (0)

#define AUSDK_Throw(err) /* NOLINT(cppcoreguidelines-macro-usage) */                               \
	do {                                                                                           \
		AUSDK_LogError("throwing %d", static_cast<int>(err));                                      \
		throw ausdk::AUException{ err };                                                           \
	} while (0)

#define AUSDK_ThrowQuietIf(cond, err) /* NOLINT(cppcoreguidelines-macro-usage) */                  \
	do {                                                                                           \
		if (cond) {                                                                                \
			throw ausdk::AUException{ err };                                                       \
		}                                                                                          \
...
#define AUSDK_ThrowQuiet(err) /* NOLINT(cppcoreguidelines-macro-usage) */                          \
	throw ausdk::AUException { err }
...
struct timebase_info {
	timebase_info()
	{
		...
		mFrequency = static_cast<double>(theTimeBaseInfo.denom) /
					 static_cast<double>(theTimeBaseInfo.numer) * 1.0e9; // NOLINT
		mToNanosNumerator = theTimeBaseInfo.numer;
		mToNanosDenominator = theTimeBaseInfo.denom;
		mInverseFrequency = 1.0 / mFrequency;
	}

	double mFrequency{};            ///< ticks per second
	double mInverseFrequency{};     ///< seconds per tick
	uint64_t mToNanosNumerator{};   ///< from mach_timebase_info.numer
	uint64_t mToNanosDenominator{}; ///< mach_timebase_info.denom

	static const timebase_info& instance()
	{
		static timebase_info info;
		return info;
	}
};
...
inline double frequency() { return timebase_info::instance().mFrequency; }

/// Returns the inverse frequency of the host timebase.
inline double inverse_frequency() { return timebase_info::instance().mInverseFrequency; }
