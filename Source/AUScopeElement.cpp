...
{
	if (mUseIndexedParameters) {
		AUSDK_ThrowIf(paramID >= mIndexedParameters.size(), kAudioUnitErr_InvalidParameter);
	...
	if (mUseIndexedParameters) {
		...
		if (i == mParameters.end()) {
			if (mAudioUnit.IsInitialized() && !okWhenInitialized) {
				...
				// AUSDK_THROW(kAudioUnitErr_InvalidParameter);
				AUSDK_LogError(
...
AUIOElement::AUIOElement(AUBase& audioUnit) : AUElement(audioUnit), mWillAllocate(true)
{
	mStreamFormat = AudioStreamBasicDescription{ .mSampleRate = AUBase::kAUDefaultSampleRate,
		...
		.mFormatFlags =
			kAudioFormatFlagsNativeFloatPacked | kAudioFormatFlagIsNonInterleaved, // NOLINT
...
//_____________________________________________________________________________
//
bool AUScope::RestoreElementNames(CFDictionaryRef inNameDict) const
{
